...
{
    class PainterBookConfigure : IEntityTypeConfiguration<PainterBook>
    {
        public void Configure(EntityTypeBuilder<PainterBook> builder)
        {
            builder
                .HasKey(pb => new { pb.BookId, pb.PainterId });
            builder
                .HasOne(bp => bp.Book)
                .WithMany(b => b.PainterBooks)
                .HasForeignKey(pb => pb.BookId)
                .OnDelete(DeleteBehavior.Cascade);
            builder
                .HasOne(pb => pb.Painter)
                .WithMany(p => p.PainterBooks)
                .HasForeignKey(pb => pb.PainterId)
                .OnDelete(DeleteBehavior.Cascade);
